...
# UUID v4 pattern – image_id values generated by the upload endpoint always
# match this format, so any deviation indicates an invalid or malicious input.
_UUID_RE = re.compile(
...
def _validate_image_id(image_id: str) -> None:
    """Raise 400 if *image_id* does not look like a UUID v4 string."""
...
def find_uploaded_file(image_id: str) -> str:
    ...
    What it does:
      Validates the image_id format, then scans the uploads directory for
      a file whose stem equals *image_id* and whose extension is in the
      allowed set.
...
def _cache_path(image_id: str) -> str:
    """Return the path to the JSON cache file for *image_id* (assumes already validated)."""
...
def save_metadata(image_id: str, metadata: dict) -> None:
    ...
    What it does:
      Validates image_id, then serialises *metadata* to
